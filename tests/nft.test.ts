...
import { boolCV, noneCV, principalCV, someCV, uintCV } from "@stacks/transactions";
...
const getOwner = (id: number) => {
  return simnet.callReadOnlyFn(CONTRACT, "get-owner", [uintCV(id)], simnet.deployer).result
}
...
describe("transfer", () => {
  it("fails when called by someone who is not NFT owner", () => {
    ...
    const result = simnet.mineBlock([transfer(1, owner, not_owner, not_owner)])[0].result;
